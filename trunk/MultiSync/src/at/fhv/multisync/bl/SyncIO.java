...
class SyncIO {
	 ...
	 *         otherwise
	 */
	static String setFileTime(final File file, final long time) {
	 ...
	 */
	static String renameFile(final File source, final File target) {
				...
				if (existingIsDirectory) {
					...
					if (error != null)
						return "Failed to delete existing directory \""
								+ trimTrailingSeparator(existingFile.getPath())
								+ File.separatorChar + "\":\n" + error;
				} else {
	 ...
	 */
	static String copyFile(final File source, final File target) {
	 ...
	 *         message otherwise
	 */
	static String deleteFileDir(final File file) {
		if (file.isDirectory()) {
	 ...
	 */
	private static String deleteDirTreeOperation(final File dir) {
		...
		DeleteNextDirectory: while (!fullDirs.isEmpty()) {
			...
			if (fd == marker) {
				...
				if (!success)
					errors.add("Failed to delete directory \""
							+ ed.getPath()
			...
			if (files == null) {
				errors.add("Failed to get contents of directory \""
						+ fd.getPath() + "\"");
			} else {
	 ...
	 * @return null if operation is successful; an error message otherwise
	 */
	private static String copyFileOperation(final File source, final File target) {
		...
		try {
			...
			try {
				...
				return "Failed to open source file for reading ("
						+ e.getMessage() + ")";
			}
			...
			try {
				...
				return "Failed to open target file for writing ("
						+ e.getMessage() + ")";
			}
			...
			try {
				...
				while (true) {
					final int byteCount = bis.read(byteBuffer, 0,
							SyncIO.BUFFER_SIZE);
				...
				return "Failed to copy data from source file to target file ("
						+ e.getMessage() + ")";
			}
			...
			try {
				...
				errors.add("Failed to close source file after reading ("
						+ e.getMessage() + ")");
			}
			...
			try {
				...
				errors.add("Failed to close target file after writing ("
						+ e.getMessage() + ")");
			}

			final boolean success = target.setLastModified(source
					.lastModified());
	 ...
	 */
	static Crc32Checksum getCrc32Checksum(final File file) {
		...
		try {
			try {
				...
				return new Crc32Checksum("Failed to open file for reading ("
						+ e.getMessage() + ")", 0L);
			}
			...
			try {
				...
				return new Crc32Checksum("Failed to read data from file ("
						+ e.getMessage() + ")", 0L);
			}
	 ...
	 */
	static String trimTrailingSeparator(final String path) {
	 ...
	 *         case)
	 */
	static char userCharPrompt(final String prompt, final String ops) {
	 ...
	 *            String to be printed
	 */
	static void printToErr(final String s) {
		Sync.stderr.print(s);
		Sync.stderr.flush();
	...
	static class Crc32Checksum {
		 ...
		 * 
		 * @param error
		 *            null if operation is successful; an error message
		 *            otherwise
		 * @param checksum
		 *            File CRC-32 checksum value
		 */
		Crc32Checksum(final String error, final long checksum) {
			this.error = error;
